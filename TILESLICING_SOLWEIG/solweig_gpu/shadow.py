...
def svf_calculator(patch_option,amaxvalue, a, vegdem, vegdem2, bush, scale):
    ...
    rows = a.shape[0]
    cols = a.shape[1]
    # Lean defaults: fp32 for 2D SVFs, reduced precision for large 3D mats on GPU
    svf_dtype = torch.float32
    if device.type == 'cuda':
        mats3d_dtype = torch.bfloat16 if torch.cuda.is_bf16_supported() else torch.float16
    else:
        mats3d_dtype = torch.float32
    
    svf = torch.zeros([rows, cols], device=device, dtype=svf_dtype)
    svfE = torch.zeros([rows, cols], device=device, dtype=svf_dtype)
    svfS = torch.zeros([rows, cols], device=device, dtype=svf_dtype)
    svfW = torch.zeros([rows, cols], device=device, dtype=svf_dtype)
    svfN = torch.zeros([rows, cols], device=device, dtype=svf_dtype)
    svfveg = torch.zeros((rows, cols), device=device, dtype=svf_dtype)
    svfEveg = torch.zeros((rows, cols), device=device, dtype=svf_dtype)
    svfSveg = torch.zeros((rows, cols), device=device, dtype=svf_dtype)
    svfWveg = torch.zeros((rows, cols), device=device, dtype=svf_dtype)
    svfNveg = torch.zeros((rows, cols), device=device, dtype=svf_dtype)
    svfaveg = torch.zeros((rows, cols), device=device, dtype=svf_dtype)
    svfEaveg = torch.zeros((rows, cols), device=device, dtype=svf_dtype)
    svfSaveg = torch.zeros((rows, cols), device=device, dtype=svf_dtype)
    svfWaveg = torch.zeros((rows, cols), device=device, dtype=svf_dtype)
    svfNaveg = torch.zeros((rows, cols), device=device, dtype=svf_dtype)
    ...
    total_patches = int(torch.sum(aziinterval).item())
    shmat = torch.zeros((rows, cols, total_patches), device=device, dtype=mats3d_dtype)
    vegshmat = torch.zeros((rows, cols, total_patches), device=device, dtype=mats3d_dtype)
    vbshvegshmat = torch.zeros((rows, cols, total_patches), device=device, dtype=mats3d_dtype)
