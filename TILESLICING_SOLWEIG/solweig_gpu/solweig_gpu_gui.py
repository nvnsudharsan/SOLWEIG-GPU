...
class SOLWEIGApp(QWidget):
    ...
    def _met_inputs_layout(self):
        ...
        self.met_source.addItems(["ERA5 (netcdf)", "COSMO (netcdf)"])
        ...
        layout.addRow(self._label_with_help("Source", "Choose the type of gridded meteorological input."), self.met_source)
        layout.addRow(self._label_with_help("Path", "Folder containing ERA5/COSMO input data."), self._with_button(self.met_path_input, self.met_path_button))
    ...
    def toggle_met_selector(self, source):
        if "COSMO (netcdf)" in source:
            self.met_path_input.setPlaceholderText("Select the folder for COSMO data")
        else:
    ...
    def browse_met_source(self):
        ...
        if path:
            if "COSMO" in source:
                cosmo_files = [f for f in os.listdir(path) if f.startswith("cosmo")]
                if not cosmo_files:
                    QMessageBox.warning(self, "No COSMO Files", "The selected folder does not contain any files starting with 'cosmo'.")
                    return
                self.log_output.append(f"📂 Found {len(cosmo_files)} COSMO files in {path}:")
    ...
    def _monitor_required_inputs(self):
        fields = [self.base_path_input, self.met_path_input, self.start_time, self.end_time]
        for field in fields:
    ...
    def _check_run_button(self):
        required = [
            self.base_path_input.text().strip(),
            self.met_path_input.text().strip(),
    ...
    def run_solweig(self):
        source = self.met_source.currentText()
        data_source_type = source.split()[0]
        params = dict(
            input_data_path=self.base_path_input.text(),
            output_path=self.base_path_input.text(),
