...
def ensure_scalar(x, device=None):
    """Return a 0-D tensor (scalar). If x is a tensor with dims, take its mean."""
...
def sunonsurface_2018a(azimuthA, scale, buildings, shadow, sunwall, first, second, aspect, walls, Tg, Tgwall, Ta,
                       emis_grid, ewall, alb_grid, SBC, albedo_b, Twater, landuse, landcover):
    ...
    az_val = float(azimuth.item()) if isinstance(azimuth, torch.Tensor) and azimuth.ndim == 0 else float(azimuth)

    index = 0
    f = buildings
    Lup = SBC * emis_grid * (Tg * shadow + Ta + 273.15) ** 4 - SBC * emis_grid * (Ta + 273.15) ** 4
    # If land cover indicates water, set Tg on water pixels to (Twater - Ta)
    # Use scalar assignment to avoid shape mismatch with masked indexing
    try:
        if int(landcover.item()) == 1:
            delta = float(ensure_scalar(Twater - Ta, device=device).item())
            Tg[landuse == 3] = delta
    except Exception:
        # Fallback: be permissive but safe
        if (isinstance(landcover, (int, float)) and landcover == 1) or (
            isinstance(landcover, torch.Tensor) and landcover.ndim == 0 and int(landcover.item()) == 1):
            delta = float(ensure_scalar(Twater - Ta, device=device).item())
            Tg[landuse == 3] = delta
    ...
    # Scale search radii to pixel units and ensure Python ints for flow control
    first = int(torch.round(ensure_scalar(first, device=device) * scale).item())
    if first < 1:
        first = 1
    second = int(torch.round(ensure_scalar(second, device=device) * scale).item())
    ...
    for n in range(second):
        if (pibyfour <= az_val < threetimespibyfour) or (fivetimespibyfour <= az_val < seventimespibyfour):
    ...
    if float(azilow.item()) >= 0 and float(azihigh.item()) < 2 * math.pi:
        facesh = (torch.logical_or(aspect < azilow, aspect >= azihigh).float() - wallbol + 1)
    elif float(azilow.item()) < 0 and float(azihigh.item()) <= 2 * math.pi:
...
def gvf_2018a(wallsun, walls, buildings, scale, shadow, first, second, dirwalls, Tg, Tgwall, Ta, emis_grid, ewall,
              alb_grid, SBC, albedo_b, rows, cols, Twater, landuse, landcover):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    for j in torch.arange(0, len(azimuthA), device=device):
        _, gvfLupi, gvfalbi, gvfalbnoshi, gvf2 = sunonsurface_2018a(
            ...
            Ta, emis_grid, ewall, alb_grid, SBC, albedo_b, Twater, landuse, landcover
        )
...
def Kside_veg_v2022a(radI, radD, radG, shadow, svfS, svfW, svfN, svfE, svfEveg, svfSveg, svfWveg, svfNveg,
                     ...
                     transVeg, rows, cols, asvf, shmat, vegshmat, vbshvegshmat):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    if anisotropic_diffuse == 1:
        ...
        if cyl == 1:
            for idx in range(patch_azimuth.shape[0]):
                ...
                diff_slice = shmat[:, :, idx] - (1 - vegshmat[:, :, idx]) * (1 - transVeg)
                KsideD += diff_slice * lumChi[idx] * anglIncC * steradian[idx]
            ...
            for idx in range(patch_azimuth.shape[0]):
                if (patch_azimuth[idx] > 360) or (patch_azimuth[idx] <= 180):
                    ...
                    diff_slice = shmat[:, :, idx] - (1 - vegshmat[:, :, idx]) * (1 - transVeg)
                    diffRadE += diff_slice * lumChi[idx] * anglIncE * steradian[idx]
                ...
                if (patch_azimuth[idx] > 90) and (patch_azimuth[idx] <= 270):
                    ...
                    diff_slice = shmat[:, :, idx] - (1 - vegshmat[:, :, idx]) * (1 - transVeg)
                    diffRadS += diff_slice * lumChi[idx] * anglIncS * steradian[idx]
                ...
                if (patch_azimuth[idx] > 180) and (patch_azimuth[idx] <= 360):
                    ...
                    diff_slice = shmat[:, :, idx] - (1 - vegshmat[:, :, idx]) * (1 - transVeg)
                    diffRadW += diff_slice * lumChi[idx] * anglIncW * steradian[idx]
                ...
                if (patch_azimuth[idx] > 270) or (patch_azimuth[idx] <= 90):
                    ...
                    diff_slice = shmat[:, :, idx] - (1 - vegshmat[:, :, idx]) * (1 - transVeg)
                    diffRadN += diff_slice * lumChi[idx] * anglIncN * steradian[idx]
...
def clearnessindex_2013b(zen, jday, Ta, RH, radG, location, P):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    # Normalize inputs; reuse names to avoid parallel variants
    P = float(ensure_scalar(P, device=device).item())
    jday = int(float(ensure_scalar(jday, device=device).item()))
    Ta = ensure_tensor(Ta, device=device)
    RH = ensure_tensor(RH, device=device)
    radG = ensure_tensor(radG, device=device)
    ...
    D = sun_distance(torch.tensor(jday, device=device, dtype=torch.float32))
...
def diffusefraction(radG, altitude, Kt, Ta, RH):
    """Compute direct (radI) and diffuse (radD) shortwave components.

    Accepts tensors or Python scalars. Internally converts values used in
    Python conditionals to plain floats to avoid ambiguous tensor booleans.
    Returns tensors on the active device.
    """
    import math

    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

    # Normalize inputs (overwrite names to avoid parallel variants)
    radG = ensure_tensor(radG, device=device).to(torch.float32)
    altitude = float(ensure_scalar(altitude, device=device).item())
    Kt = float(ensure_scalar(Kt, device=device).item())
    Ta = float(ensure_scalar(Ta, device=device).item())
    RH = float(ensure_scalar(RH, device=device).item())

    alfa = torch.tensor(altitude * (math.pi / 180.0), device=device)

    # Handle missing Ta/RH (-999 or NaN) using simple Reindl et al. relations
    if (Ta <= -999.0) or (RH <= -999.0) or math.isnan(Ta) or math.isnan(RH):
        if Kt <= 0.3:
        ...
        if Kt <= 0.3:
            radD = radG * (1 - 0.232 * Kt + 0.0239 * sin_alfa - 0.000682 * Ta + 0.0195 * RHn)
        elif 0.3 < Kt < 0.78:
            radD = radG * (1.329 - 1.716 * Kt + 0.267 * sin_alfa - 0.00357 * Ta + 0.106 * RHn)
        else:
            radD = radG * (0.426 * Kt - 0.256 * sin_alfa + 0.00349 * Ta + 0.0734 * RHn)

    # Direct normal irradiance component projected by sun altitude
    sin_alfa = torch.sin(alfa)
    sin_alfa = torch.where(sin_alfa == 0, torch.tensor(1e-6, device=device), sin_alfa)
    radI = (radG - radD) / sin_alfa

    # Corrections for low sun altitudes (20130307)
    zero = torch.tensor(0.0, device=device)
    radI = torch.where(radI < 0, zero, radI)
    if altitude < 1:
        radI = torch.where(radI > radG, radG, radI)
...
def shadowingfunction_wallheight_23(a, vegdem, vegdem2, azimuth, altitude, scale, amaxvalue, bush, walls, aspect):
    ...
    azilow = azimuth - torch.pi / 2
    azihigh = azimuth + torch.pi / 2
    if float(azilow.item()) >= 0 and float(azihigh.item()) < 2 * math.pi:    # 90 to 270  (SHADOW)
        facesh = torch.logical_or(aspect < azilow, aspect >= azihigh).float() - wallbol + 1
    elif float(azilow.item()) < 0 and float(azihigh.item()) <= 2 * math.pi:    # 0 to 90
        azilow += 2 * torch.pi
        facesh = torch.logical_or(aspect > azilow, aspect <= azihigh).float() * -1 + 1
    elif float(azilow.item()) > 0 and float(azihigh.item()) >= 2 * math.pi:    # 270 to 360
        azihigh -= 2 * torch.pi
        facesh = torch.logical_or(aspect > azilow, aspect <= azihigh).float() * -1 + 1
...
def Perez_v3(zen, azimuth, radD, radI, jday, patchchoice, patch_option):
    ...
    if float((altitude * rad2deg).item()) >= 10:
        AirMass = 1 / torch.sin(altitude)
    elif float((altitude * rad2deg).item()) < 0:
        AirMass = 1 / torch.sin(altitude) + 0.50572 * torch.pow((altitude * rad2deg) + 6.07995, -1.6364)
    else:
...
def model1(sky_patches, esky, Ta):
    ...
    esky_val = ensure_scalar(esky, device=device)
    ...
    ln_u_prec = esky_val / b_c - a_c / b_c - 0.5
...
def model2(sky_patches, esky, Ta):
    ...
    esky_band = 1 - (1 - esky_val) * torch.exp(b_c * (1.7 - (1 / torch.cos(skyzen * deg2rad))))
    p_alt = sky_patches[:, 0]
    patch_emissivity = torch.zeros((p_alt.shape[0]), device=device)
...
def model3(sky_patches, esky, Ta):
    ...
    esky_band = 1 - (1 - esky_val) ** (1 / (b_c * torch.cos(skyzen * deg2rad)))
    p_alt = sky_patches[:, 0]
    patch_emissivity = torch.zeros((p_alt.shape[0]), device=device)
...
def Lcyl_v2022a(esky, sky_patches, Ta, Tgwall, ewall, Lup, shmat, vegshmat, vbshvegshmat, solar_altitude, solar_azimuth, rows, cols, asvf):
    ...
    # Reduce possibly 2D/field inputs to scalars where patch-level 1D arrays are expected
    Ta_scalar   = ensure_scalar(Ta, device=device)
    esky_scalar = ensure_scalar(esky, device=device)

    # Stefan-Boltzmann's Constant
    SBC = torch.tensor(5.67051e-8, device=device).clone().detach()

    # Sky longwave radiation from emissivity based on Prata (1996)
    Ldown_prata = (esky_scalar * SBC * ((Ta_scalar + 273.15) ** 4))
    ...
    steradian = torch.zeros(patch_altitude.shape[0], device=device)
    if skyalt.shape[0] > 0:
        lower_bounds = torch.zeros_like(skyalt, dtype=torch.float32)
        upper_bounds = torch.zeros_like(skyalt, dtype=torch.float32)

        for idx in range(skyalt.shape[0]):
            alt = skyalt[idx]
            ...
            if idx == 0:
                if skyalt.shape[0] == 1:
                    ...
                    lower = torch.clamp(alt - (skyalt[idx + 1] - alt) / 2, min=0.0)
            else:
                lower = (skyalt[idx - 1] + alt) / 2

            if idx == skyalt.shape[0] - 1:
                if skyalt.shape[0] == 1:
                    upper = torch.tensor(90.0, device=device)
                else:
                    upper = torch.clamp(alt + (alt - skyalt[idx - 1]) / 2, max=90.0)
            else:
                upper = (alt + skyalt[idx + 1]) / 2

            lower_bounds[idx] = lower
            upper_bounds[idx] = upper

        for idx in range(skyalt.shape[0]):
            band_mask = (patch_altitude == skyalt[idx])
            if not torch.any(band_mask):
                continue

            delta_azi = (360.0 / skyalt_c[idx].to(torch.float32)) * deg2rad
            band_area = delta_azi * (torch.sin(upper_bounds[idx] * deg2rad) - torch.sin(lower_bounds[idx] * deg2rad))
            steradian[band_mask] = band_area
    ...
    for altitude in skyalt:
        ...
        Ldown[patch_altitude == altitude] = ((temp_emissivity * SBC * ((Ta_scalar + 273.15) ** 4)) / torch.tensor(np.pi, device=device)) * steradian[patch_altitude == altitude] * torch.sin(altitude * deg2rad)
        Lside[patch_altitude == altitude] = ((temp_emissivity * SBC * ((Ta_scalar + 273.15) ** 4)) / torch.tensor(np.pi, device=device)) * steradian[patch_altitude == altitude] * torch.cos(altitude * deg2rad)
        Lnormal[patch_altitude == altitude] = ((temp_emissivity * SBC * ((Ta_scalar + 273.15) ** 4)) / torch.tensor(np.pi, device=device)) * steradian[patch_altitude == altitude]
...
def Lside_veg_v2022a(svfS, svfW, svfN, svfE, svfEveg, svfSveg, svfWveg, svfNveg, svfEaveg, svfSaveg, svfWaveg, svfNaveg, azimuth, altitude, Ta, Tw, SBC, ewall, Ldown, esky, t, F_sh, CI, LupE, LupS, LupW, LupN, anisotropic_longwave):
    ...
    if float(altitude.item()) > 0:  # daytime
...
def Solweig_2022a_calc(i, dsm, scale, rows, cols, svf, svfN, svfW, svfE, svfS, svfveg, svfNveg, svfEveg, svfSveg,
                       ...
                       landcover, landuse,hsdiff, dectime, altmax, dirwalls, walls, cyl, elvis, Ta, RH, radG, radD, radI, P,
                       amaxvalue, bush, Twater, TgK, Tstart, alb_grid, emis_grid, TgK_wall, Tstart_wall, TmaxLST,
                       TmaxLST_wall, first, second, svfalfa, svfbuveg, transVeg, firstdaytime, timeadd, timestepdec, Tgmap1,
                       Tgmap1E, Tgmap1S, Tgmap1W, Tgmap1N, CI,  TgOut1, shmat, vegshmat, vbshvegshmat, anisotropic_sky, asvf, patch_option):
    ...
    if float(altitude.item()) > 0: # # # # # # DAYTIME # # # # # #
        ...
        I0, CI, Kt, I0et, CIuncorr = clearnessindex_2013b(
                                     ...
                                     location,
                                     P)
        CI = torch.minimum(ensure_tensor(CI, device=device), torch.tensor(1.0, device=device))
        ...
        if anisotropic_sky == 1:
            ...
            for idx in range(lv.shape[0]):
                # diffsh = sh - (1 - veg) * (1 - transVeg)
                diff_slice = shmat[:, :, idx] - (1 - vegshmat[:, :, idx]) * (1 - transVeg)
                aniLum += diff_slice * lv[idx,2]
        ...
        CI_Tg = torch.minimum(ensure_tensor(CI_Tg, device=device), torch.tensor(1.0, device=device))

        deg2rad = np.pi / 180
        radG0 = radI0 * (torch.sin(altitude * deg2rad)) + _
        CI_TgG = (radG / radG0) + (1 - corr)
        CI_TgG = torch.minimum(ensure_tensor(CI_TgG, device=device), torch.tensor(1.0, device=device))
        ...
        Keast, Ksouth, Kwest, Knorth, KsideI, KsideD, Kside = Kside_veg_v2022a(radI, radD, radG, shadow, svfS, svfW, svfN, svfE,
                    ...
                    KupN, cyl, lv, anisotropic_sky, transVeg, rows, cols, asvf, shmat, vegshmat, vbshvegshmat)
        ...
        if landcover == 1:
            Lup[landuse == 3] = (SBC * 0.98 * (Twater + 273.15) ** 4).float()  # nocturnal Water temp
    ...
    if anisotropic_sky == 1:
        ...
        if float(altitude.item()) < 0:
            CI = deepcopy(CI)

        CI_val = float(ensure_scalar(CI, device=device).item())
        if CI_val < 0.95:
            esky_c = ensure_tensor(CI, device=device) * esky + (1 - CI_val) * 1.0
            esky = esky_c
        ...
        CI_val = float(ensure_scalar(CI, device=device).item())
        if CI_val < 0.95:  # non - clear conditions
            c = 1 - CI_val
    ...
    if cyl == 0 and anisotropic_sky == 1:
        Least_tot = Least + Least_
        Lwest_tot = Lwest + Lwest_
        Lnorth_tot = Lnorth + Lnorth_
        Lsouth_tot = Lsouth + Lsouth_
    else:
        Least_tot = Least
        Lwest_tot = Lwest
        Lnorth_tot = Lnorth
    ...
    # and Martin and Berdahl (1984) (anisotropic sky longwave)
        ...
        Sstr = absK * (Kside * Fcyl + (Kdown + Kup) * Fup + (Knorth + Keast + Ksouth + Kwest) * Fside) + absL * \
                        ((Ldown + Lup) * Fup + Lside * Fcyl + (Lnorth_tot + Least_tot + Lsouth_tot + Lwest_tot) * Fside)
    # Knorth = nan Ksouth = nan Kwest = nan Keast = nan
        ...
        Sstr = absK * ((Kdown + Kup) * Fup + (Knorth + Keast + Ksouth + Kwest) * Fside) + absL * \
                        ((Ldown + Lup) * Fup + (Lnorth_tot + Least_tot + Lsouth_tot + Lwest_tot) * Fside)

    Tmrt = torch.sqrt(torch.sqrt((Sstr / (absL * SBC)))) - 273.15
