...
After running a simulation, outputs are saved in the `output_folder/` directory within your `base_path`. The structure is organized by tile key (origin coordinates in the raster grid):
