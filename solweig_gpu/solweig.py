...
def Kup_veg_2015a(radI, radD, radG, altitude, svfbuveg, albedo_b, F_sh, gvfalb, gvfalbE, gvfalbS, gvfalbW, gvfalbN, gvfalbnosh, gvfalbnoshE, gvfalbnoshS, gvfalbnoshW, gvfalbnoshN):
    ...
    # gvfalbS = torch.tensor(gvfalbS, device=device)
    # gvfalbW = torch.tensor(gvfalbW, device=device)
    # gvfalbN = torch.tensor(gvfalbN, device=device)
    # gvfalbnosh = torch.tensor(gvfalbnosh, device=device)
    # gvfalbnoshE = torch.tensor(gvfalbnoshE, device=device)
    # gvfalbnoshS = torch.tensor(gvfalbnoshS, device=device)
    # gvfalbnoshW = torch.tensor(gvfalbnoshW, device=device)
    # gvfalbnoshN = torch.tensor(gvfalbnoshN, device=device)
...
def define_patch_characteristics(solar_altitude, solar_azimuth,
    ...
    # Changed here
    # shmat = torch.tensor(shmat, device=device)
    # vegshmat = torch.tensor(vegshmat, device=device)
...
def Lside_veg_v2022a(svfS, svfW, svfN, svfE, svfEveg, svfSveg, svfWveg, svfNveg, svfEaveg, svfSaveg, svfWaveg, svfNaveg, azimuth, altitude, Ta, Tw, SBC, ewall, Ldown, esky, t, F_sh, CI, LupE, LupS, LupW, LupN, anisotropic_longwave):
    ...
    # Convert input data to torch tensors
    # Changed here
    # svfS = torch.tensor(svfS, device=device)
    # svfW = torch.tensor(svfW, device=device)
    # svfN = torch.tensor(svfN, device=device)
    # svfE = torch.tensor(svfE, device=device)
    # svfEveg = torch.tensor(svfEveg, device=device)
    # svfSveg = torch.tensor(svfSveg, device=device)
    # svfWveg = torch.tensor(svfWveg, device=device)
    # svfNveg = torch.tensor(svfNveg, device=device)
    # svfEaveg = torch.tensor(svfEaveg, device=device)
    # svfSaveg = torch.tensor(svfSaveg, device=device)
    # svfWaveg = torch.tensor(svfWaveg, device=device)
    # svfNaveg = torch.tensor(svfNaveg, device=device)
    # Changed here
...
def Solweig_2022a_calc(i, dsm, scale, rows, cols, svf, svfN, svfW, svfE, svfS, svfveg, svfNveg, svfEveg, svfSveg,
    ...
    # dirwalls = torch.tensor(dirwalls, device=device)
    # walls = torch.tensor(walls, device=device)
    # buildings = torch.tensor(buildings, device=device)
    # Ta = torch.tensor(Ta, device=device)
    # RH = torch.tensor(RH, device=device)
    ...
    # svfalfa = torch.tensor(svfalfa, device=device)
    # svfbuveg = torch.tensor(svfbuveg, device=device)
    # diffsh = torch.tensor(diffsh, device=device)

    # shmat = torch.tensor(shmat, device=device)
    # vegshmat = torch.tensor(vegshmat, device=device)
    # vbshvegshmat = torch.tensor(vbshvegshmat, device=device)

    # asvf = torch.tensor(asvf, device=device)
